...
type GetUserGardenByIDURL struct {
	GardenID int64
...
func (o *GetUserGardenByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/api/v1/gardens/{garden_id}"

	gardenID := swag.FormatInt64(o.GardenID)
	if gardenID != "" {
		_path = strings.Replace(_path, "{garden_id}", gardenID, -1)
	} else {
		return nil, errors.New("gardenId is required on GetUserGardenByIDURL")
	}
...
func (o *GetUserGardenByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetUserGardenByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetUserGardenByIDURL")
	}
