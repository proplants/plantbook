...
func (pg *PG) StorePlant(ctx context.Context, plant *models.UserPlant) (*models.UserPlant, error) {
	...
	// interval := plant.WateringInterval.String()
...
func (pg *PG) ListUserPlants(ctx context.Context, userID int64, limit int64, offset int64) ([]*models.UserPlant, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&userPlant.ID, &userPlant.UserID, &userPlant.PlantReferenceID, &userPlant.GardenID, &userPlant.PlantingDate,
			&interval, &userPlant.LastWatering, &userPlant.NextWatering, &userPlant.PhotoUrls,
			&userPlant.Name, &userPlant.Description, &userPlant.CreatedAt, &userPlant.ModifiedAt)
		if err != nil {
